...
public class Product {
    ...
    static List<Product> GetProductsFromDataTable(DataTable data) {
        if (data != null) {
    ...
    public static List<Product> GetProducts(int categoryID) {
        DataTable prodData = DataHelper.ProcessSelectCommand("SELECT * FROM [Products] WHERE ([CategoryID] = {0})", categoryID);
        return GetProductsFromDataTable(prodData);
    }
}